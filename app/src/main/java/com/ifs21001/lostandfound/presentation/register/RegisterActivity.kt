package com.ifs21001.lostandfound.presentation.register
...
import com.ifs21001.lostandfound.databinding.ActivityRegisterBinding
...
class RegisterActivity : AppCompatActivity() {
    ...
    private fun observeRegister(name: String, email: String, password: String){
        viewModel.register(
            ...
            email,
            password
        ).observe(this) { result: MyResult<DelcomResponse> ->
            if (result != null) {
                when (result) {
                    is MyResult.Loading -> {
                        showLoading(true)
